    ...
    for j in range(5):
        for i in range(5):
            ...
            if board[i_j_map][ENEMY_VOL] == 0:
                ...
                print(board[i_j_map][0], board[i_j_map][1],end=" ")

            if board[i_j_map][2] == 0:
                print("  |",end="")
            else :
                print(board[i_j_map][2],"|",end="")
        print()
...
def rule_setumei():
    ...
    print("| 1 3 G |・左から敵の数、敵の強さ、ガスの有無(G)")
    print("| 0 0 1 |・左からボスの有無(B)、沈没済か(×)、資材の数")
    print("| 0     |・武器の有無(W)")
    print("(この場合は1マスに強さ3の敵とガスと資材が一つずつ存在している)")
    print("5:日数経過")
    print("・スタミナが0になると一日が経過")
    print("・一日経過するとスタミナが全回復し、HPが2~4ランダムな値回復")
    print("・4日,7日10日ごとに島の辺が沈んでいく(南,東,北の順)")
    print("・沈む際、そのマスにあったP以外の要素は一番近いマスに移動される")
    print("6:各要素の出現")
