...
def without_zero(player_place,board):
...
def put_enemies(board, day, player_p):
    if day == 5:
        #ボスと資材の配置
        boss_p = without_zero(player_p, board)
        while board[boss_p][MATTERIAL_PLACE] >= 1 or boss_p == 0:#なんかバグるから資材と武器は座標0に設置しない
            boss_p = without_zero(player_p, board)
        board[boss_p][BOSS_PLACE] = BOSS_IRU
        board[boss_p][MATTERIAL_PLACE] += 1
        #武器をボスと重ならないように配置
        weapon_p = without_zero(player_p, board)
        while weapon_p == boss_p or weapon_p == 0:#なんかバグるから資材と武器は座標0に設置しない
            weapon_p = without_zero(player_p, board)
        board[weapon_p][WEAPON] = BUKI_ARU
    else:
        #資材の配置
        without_matt = without_zero(player_p, board)
        while board[without_matt][MATTERIAL_PLACE] >= 1 or without_matt == 0:#なんかバグるから資材と武器は座標0に設置しない
            without_matt = without_zero(player_p, board)
        board[without_zero(player_p, board)][MATTERIAL_PLACE] += 1
    ...
    while True:
        if day==10:
            if player[2]==10:
                ...
                #print("⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻")
                print(YELLOW+" _______  _______  __   __  _______  _______  ___      _______  _______  ______"+IRO_USIRO)
                print(YELLOW+"|       ||   _   ||  |_|  ||       ||       ||   |    |       ||   _   ||    _ |"+IRO_USIRO)
                print(YELLOW+"|    ___||  | |  ||       ||    ___||       ||   |    |    ___||  | |  ||   | ||"+IRO_USIRO)
                print(YELLOW+"|   | __ |  |_|  ||       ||   |___ |       ||   |    |   |___ |  |_|  ||   |_||"+IRO_USIRO)
                print(YELLOW+"|   ||  ||       ||       ||    ___||      _||   |___ |    ___||       ||    __ |"+IRO_USIRO)
                print(YELLOW+"|   |_| ||   _   || ||_|| ||   |___ |     |_ |       ||   |___ |   _   ||   |  ||"+IRO_USIRO)
                print(YELLOW+"|_______||__| |__||_|   |_||_______||_______||_______||_______||__| |__||___|  ||"+IRO_USIRO)
                break
            else:
                #print("₋₋₋₋₋₋₋₋₋₋₋₋₋")
                #print("| "+RED+"GAME OVER"+IRO_USIRO+" |")
                #print("⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻")
                print(RED+" _______  _______  __   __  _______  _______  __   __  _______  ______"+IRO_USIRO)
                print(RED+"|       ||   _   ||  |_|  ||       ||       ||  | |  ||       ||    _ |"+IRO_USIRO)
                print(RED+"|    ___||  | |  ||       ||    ___||   _   ||  |_|  ||    ___||   | ||"+IRO_USIRO)
                print(RED+"|   | __ |  |_|  ||       ||   |___ |  | |  ||       ||   |___ |   |_||"+IRO_USIRO)
                print(RED+"|   ||  ||       ||       ||    ___||  |_|  ||       ||    ___||    __ |"+IRO_USIRO)
                print(RED+"|   |_| ||   _   || ||_|| ||   |___ |       | |     | |   |___ |   |  ||"+IRO_USIRO)
                print(RED+"|_______||__| |__||_|   |_||_______||_______|  |___|  |_______||___|  ||"+IRO_USIRO)
                break   
        else:
            day+=1   
            if day==4:
