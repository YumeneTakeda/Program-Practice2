...
def put_enemies(board, day, player_p):
    ...
    if board[str_2_enemy][ENEMY_STR] >= 9:
        board[str_2_enemy][ENEMY_STR] = 9
    str_3_enemy = without_rand(player_p, board)
    board[str_3_enemy][ENEMY_VOL] += 1
    board[str_3_enemy][ENEMY_STR] += 3
    if board[str_3_enemy][ENEMY_STR]>=9:
        board[str_3_enemy][ENEMY_STR]=9
...
def Move(player, way, Board):
    ...
    if way == "W" or way == "w":
        ...
        print("| "+BLUE+'1'+IRO_USIRO+" "+BLUE+'3'+IRO_USIRO+" "+RED+'G'+IRO_USIRO +
        ...
        print("| "+GREEN+'W'+IRO_USIRO+"     |・武器の有無("+GREEN+'W'+IRO_USIRO+")")
...
def Displaydangeon(player,board,sink,day):
    ...
    for j in range(5):
        for i in range(5):
            i_j_map = j*5+i
            print("| ",end="")
            if board[i_j_map][ENEMY_VOL] == 0:
                print("    ",end="")
            else :
                print(BLUE+str(board[i_j_map][0])+IRO_USIRO, BLUE+str(board[i_j_map][1])+IRO_USIRO,end=" ")
        ...
        for i in range(5):
            i_j_map = j*5+i
            print("| ",end="")
            if board[i_j_map][6] == 0:
                print("  ",end="")
            else:
                print(GREEN+str(board[i_j_map][6])+IRO_USIRO,"",end="")
